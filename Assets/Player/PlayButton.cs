...
public class PlayButton
{
	...
	public ButtonStatePP _state = ButtonStatePP.Normal; //State of our button
	...
	private Texture2D CurrentTextur
	{
		get
		{
			switch (_state)
			{
			case ButtonStatePP.Normal:
				return _texNormal;
			case ButtonStatePP.Hover:
				return _texHover;
			case ButtonStatePP.Active:
				return _texActive;
	...
	public PlayButton (Vector2 pos, Vector2 size, Texture2D tNormal, Texture2D tHover, Texture2D tActive, Vector2 Centr, float Radius)
	{
		SetPosition(pos);
		SetSize(size);
		_texNormal = tNormal;
	...
	public bool Update (Vector2 mousePos, bool pressed)
	{
		...
		if ((Mathf.Pow((mousePos.x - OldCentr.x),2) + Mathf.Pow((mousePos.y - OldCentr.y),2)) <= (Mathf.Pow(_Radius,2)))
		{
			...
			ButtonStatePP oldState = _state;
			_state = pressed ? ButtonStatePP.Active:ButtonStatePP.Hover;
			...
			{*/
				...
				if (oldState == ButtonStatePP.Hover && _state == ButtonStatePP.Active)
				{
					_state = ButtonStatePP.Active;
